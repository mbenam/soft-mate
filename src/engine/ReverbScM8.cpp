...
// MODIFIED COPY of DaisySP-LGPL's reverbsc.cpp (LGPL 2.1 section 2a notice).
// Vendored and changed by the soft-mate project on 2026-08-14; every change is
// marked "soft-mate" inline and the full list is in ReverbScM8.h.
...
int ReverbScM8::Init(float sr)
{
    i_sample_rate_ = sr;
    sample_rate_   = sr;
    feedback_      = 0.97;
    lpfreq_        = 10000;
    i_pitch_mod_   = 1;   /* soft-mate: stock default; SetPitchMod overrides */
    /* soft-mate: reset these to stock here too, not just at construction.
       InitDelayLine reads room_size_ when it computes each line's initial read
       position, so an Init() inherited from a previous song's setting would
       seed different state than a fresh engine gets -- which is precisely the
       cross-song divergence LOAD_SONG re-inits to avoid (ARCHITECTURE.md
       invariant 11, caught by test L9). */
    room_size_      = 1.0f;
    mod_rate_scale_ = 1.0f;
    i_skip_init_   = 0;
    damp_fact_     = 1.0;
    prv_lpfreq_    = 0.0;
    init_done_     = 1;
    ...
    for(i = 0; i < 8; i++)
    {
        if(n_bytes > M8_REVERBSC_MAX_SIZE)
            return 1;
        delay_lines_[i].buf = (aux_) + n_bytes;
        InitDelayLine(&delay_lines_[i], i);
        /* soft-mate: advance by SAMPLES. Upstream added a byte count to a
           float* here, spacing the lines 4x further apart than needed and
           leaving almost no room in aux_. Audio is unaffected -- the lines
           never overlapped -- but the recovered space is what allows the
           buffers below to be sized for kMaxPitchMod. */
        n_bytes += DelayLineMaxSamples(sr, kMaxPitchMod, i);
    }
    return 0;
}
...
void ReverbScM8::NextRandomLineseg(ReverbScDl *lp, int n)
{
    ...
    /* soft-mate: MOD FREQ scales the random-segment rate. */
    lp->rand_line_cnt
        = (int)((sample_rate_ / (kReverbParams[n][2] * mod_rate_scale_)) + 0.5);
    if(lp->rand_line_cnt < 1)
        lp->rand_line_cnt = 1;
    ...
    nxt_del = (kReverbParams[n][0] * room_size_)
              + (nxt_del * (float)i_pitch_mod_);   /* soft-mate: ROOM SIZE */
...
int ReverbScM8::InitDelayLine(ReverbScDl *lp, int n)
{
    float read_pos;
    /* int     i; */

    /* calculate length of delay line */
    /* soft-mate: sized for kMaxPitchMod, not 1, so raising MOD DEPTH
       cannot walk the read pointer outside the allocation. Room size only
       ever shrinks the delay, so 1.0 is its worst case. */
    lp->buffer_size = DelayLineMaxSamples(sample_rate_, kMaxPitchMod, n);
    lp->dummy       = 0;
    lp->write_pos   = 0;
    /* set random seed */
    ...
    read_pos     = (kReverbParams[n][0] * room_size_)
                   + (read_pos * (float)i_pitch_mod_);   /* soft-mate: ROOM SIZE */
...
int ReverbScM8::Process(const float &in1,
                      const float &in2,
                      float *      out1,
                      float *      out2)
{
