...
| [`lfo_rate`](level.md) | modulation-rate reader (Python) | Tracks the pitch wobble of a ModFX capture by short-time spectral peak and reports `NOT RESOLVED` with its cycle count and peak-to-median ratio rather than a number it cannot support. §UI-33. |
