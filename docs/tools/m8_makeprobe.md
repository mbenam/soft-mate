...
- **`--type sampler` probes are SILENT on hardware — measured 2026-08-19 (fw 6.5.2, COM3), cause not found.** A probe generated with `--type sampler --sample-path /Samples/SHAPER.WAV --volume 0x7F` loads, shows `SAMPLE SHAPER` on the INSTRUMENT screen, and produces **nothing**: peak `0.0038` (the noise floor) via PLAY *and* via `--keyjazz`. Ruled out, each by measurement: the sample is missing (`LOAD SAMPLE` browser lists `/Samples/SHAPER.WAV` and the cursor lands on it); the WAV format (`MONOREF.WAV`, 48 kHz stereo and proven on this device by the pan/stereo work, is **equally silent** when assigned to the same instrument); the trigger path (keyjazz bypasses the sequencer, still silent); and the rig (a `--type wavsynth` probe from the same generator, same `--volume 0x7F`, captured **peak 0.357** immediately afterwards). Field reads are all sane: `PLAY 00FWD`, `START 00`, `LENGTH FF`, `SLICE 00OFF`, `FILTER 00OFF`, `DRY C0`. **The tool's own round-trip check passes** — it verifies `sample_path` and the synth params reached the file, which they did, and cannot see that the result makes no sound. **One concrete lead, not chased:** the file carries `volume = 0x7F` but the device's INSTRUMENT screen shows `AMP 00`, so for the sampler the file's `volume` byte is evidently *not* the screen's AMP — which is the same conflation flagged in `SongIO.cpp:888` (`s.amp = sp.volume`) and in `status.md`'s AMP placeholder. Note also `verifyRoundTrip` has a standing special case mapping a sampler's default `--volume 0xE0` to a stored `0x00`, which suggests the sampler's volume handling was already known to be odd. **This blocks the LIM POST:W1-W3 characterisation**, which needs a sampler to inject a known input signal into the shaper.
