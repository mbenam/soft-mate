...
| `--record-raw <path>` | — | Write the **raw, pre-SLIP byte stream** to `<path>` — the startup read (which carries the post-reset full repaint) plus five more settled reads. Distinct from `--record-frames`, which stores *decoded* cells: this is the tap that settles whether a suspect screen is our decode or the device's transmission, and it is what diagnosed [#32](../../specs/M8_DRIVER_BUGS.md). To read one: SLIP-decode it, keep the 12-byte `0xFD` draw-char commands **in arrival order**, map each to `(y/10)*40 + x/8`, and look for a step that is not `+1`. Also prints the screen's label-match ratio. |
...
| `--pin-theme` | — | **Learn the cursor accent from the device** instead of assuming it, and write it to `hw_theme.json`. Presses a direction key and its inverse (DOWN/UP, then RIGHT/LEFT — more than one pair, because the cursor can sit where DOWN does nothing) and keeps the colour that moved. Needs no `--allow-mutation`: it only moves a cursor. Run it once per device/firmware. Every cursor read depends on this value, and a wrong one presents as the device ignoring keys — which cost a whole session before it was found ([#24](../../specs/M8_DRIVER_BUGS.md)). |
| `--cursor-color <R,G,B>` | *(`hw_theme.json`, else built-in `0,240,248` — **unconfirmed**)* | Override the accent colour by hand. Wants three values, each `0`-`255`; anything else is rejected with a message rather than silently ignored. Prefer `--pin-theme` — a measured accent and a guessed one must not look alike. |
